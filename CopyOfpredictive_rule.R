...
path_main <- "/home/knguyen/Documents/PhD/Results/"
path_restest <- paste0(path_main,"attribution/predictive_rule/")
file_path_Results=paste0(path_main,'attribution/predictive_rule/')
...
Keep.Data <- function(name.series){
  ...
  Data.Res <- Data.Res %>% dplyr::mutate(p=2*pnorm(-abs(t))) %>%
    mutate(signif=ifelse(p<Thresh,1,0),t.sign=sign(t),pop=(signif==0)*1+((signif==1) & (t.sign==-1))*2+ ((signif==1) & (t.sign==1))*3,pop.code=(signif==0)*0+((signif==1) & (t.sign==-1))*-1+ ((signif==1) & (t.sign==1))*1)
  return(Data.Res)
}
...
PredRule_ET_ErrorTest <- function(DataLearn,DataTest,b,Nbconfig){
  ...
  saveRDS(modlda, file = paste0(file_path_Results,'modlda_b',b,'.rds'))
  ...
  saveRDS(modknn, file = paste0(file_path_Results,'modknn_b',b,'.rds'))
...
for (b in 1:B){
  ...
  saveRDS(DataLearn, file = paste0(file_path_Results,"DataLearn_",b,".rds"))
  saveRDS(DataTest, file = paste0(file_path_Results,"DataTest_",b,".rds"))
  
  Res.pred <- PredRule_ET_ErrorTest(DataLearn,DataTest,b,Nbconfig)
  saveRDS(Res.pred, file = paste0(file_path_Results,"Res.pred_",b,".rds"))
...
for (b in 1:B){
  Res.pred <- readRDS(paste0(file_path_Results,"Res.pred_",b,".rds"))
  error.test.4.methods[b,] <- Res.pred$err.tot %>% unlist()
}
colnames(error.test.4.methods) <- c("lda","cart","knn","rf")
...
colMeans(error.test.4.methods)
apply(error.test.4.methods,2,sd)
apply(error.test.4.methods,2,which.min)

randomForest::importance(modrf$finalModel,type=1)
...
save(FinalTable, file = paste0(path_restest, "Final.Table.0.01.RData"))

save(Post.Prob.List, file = paste0(path_restest, "Post.Prob.List.0.01.RData"))
...
save(Post.Prob.List, file = paste0(path_restest, "Post.Prob.Listn.RData"))
