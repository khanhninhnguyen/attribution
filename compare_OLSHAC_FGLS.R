...
mod.sim <- function(heteroscedastic, autocorr, var.inno, list.param.sig, list.param.ar, list.param.ma, x.axis, individual, n, T1, noise.name){
  ...
  if(heteroscedastic == 1 & autocorr == 0){
    ...
    ma = rep(0, length(list.param.sig))
    ...
    if(x.axis == "rho"){
      ...
      ma = rep(list.param.ma[individual],length(list.param.sig))
    }
  }
  return(list(hetero = hetero, burn.in = burn.in, sigma.t = sigma.t, ar = ar, ma = ma))
}
...
case = data.frame(h = rep(c(0,1),4),
                  a = rep(1,8), 
                  n = rep(c(200,400,600,800),each = 2),
                  x.axis = rep(c("rho"),8), 
                  length.w = c(rep(30,2), rep(60,6)))
...
for (j in c(1:nrow(case))) {
  n1 = case$n[j]
  one.year = n1/2
  a = simu_performance(off.set = 0, heteroscedast = case$h[j] , autocor = case$a[j] , x.axis = as.character(case$x.axis[j]),
                       nb.sim = nb.sim, list.param.ar , list.param.sig, list.ma = list.ma,
                       noise.model = noise.list, noise.name = model.name, n = case$n[j], length.wind = case$length.w[j])
  print(j)
}

# b = get(load(file = paste0(path_results,"attribution/",hetero=0,"auto",autocor=1, x.axis="rho", y.axis = "FPR",noise.name = model.name,n=400,"1R.Data")))
...
compute_power(Res = Res, name.test = "fgls", k = 2, off.set =  seq(0, 0.3, 0.02), df1 = 397)
