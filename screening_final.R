...
screening <- function(path_code_att, path_results, window.thres, nearby_ver){
  source(paste0(path_code_att,"sliding_variance.R"))
  source(paste0(path_code_att,"support_screening.R"))
  
  data.in = get(load( file = paste0(path_results,"attribution/check_homo/six_diff_series_rm_crenel_restricted_closed_brp_",
  ...
  save(data.all, file = paste0(path_results,"attribution/screening/data.all_", window.thres,"years_", nearby_ver,"screened.RData"))
  save(list.outlier, file = paste0(path_results,"attribution/screening/list.outlier_",  window.thres,"years_", nearby_ver,"screened.RData"))
  save(sd.all, file = paste0(path_results,"attribution/screening/sd.all_",  window.thres,"years_", nearby_ver,"screened.RData"))
