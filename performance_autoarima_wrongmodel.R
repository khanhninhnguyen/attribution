...
save(Tot.res.AR, file = paste0(path_results, "attribution0/truemodel.MA1", N, ".RData"))
...
# }
# 
# save(Tot.res, file = paste0(path_results, "attribution0/truemodel.AR1.length.RData"))
# Wrong model -------------------------------------------------------------
...
save(Tot.res.AR, file = paste0(path_results, "attribution0/truemodel.AR1", N, ".RData"))
...
save(Tot.res.MA, file = paste0(path_results, "attribution0/wrongmodel.MA1", N, ".RData"))
...
save(Tot.res.MA, file = paste0(path_results, "attribution0/wrongmodel.ARMA", N, ".RData"))


# plot results ------------------------------------------------------------
## wrong model---
true.ar = get(load(file = paste0(path_results,  "attribution0/truemodel.MA1", N, ".RData")))
...
## length ---
true.ar = get(load(file = paste0(path_results, "attribution0/truemodel.ARMA1.length.RData")))
true.model = "AR"
t.true.ar = as.data.frame(sapply(c(1:4), function(x) sapply(c(1:nb.sim), function(y) true.ar[[x]][[y]]$t.table$`t value`[1] ))) 
colnames(t.true.ar) = as.factor(length.list)
sapply(c(1:4), function(x) table(abs(t.true.ar[,x])>1.96))
d = t.true.ar %>% reshape2::melt()
p1 = ggplot(data = d, aes(x = value, col = variable))+
  theme_bw()+
  stat_ecdf()+
  ylab("CDF")+
  xlab("t-value")
ggsave(filename = paste0(path_results, "attribution0/t_val_n_",true.model,".jpg"), plot = p1)

t.true.ar = as.data.frame(sapply(c(1:4), function(x) sapply(c(1:nb.sim), function(y) true.ar[[x]][[y]]$t.table$`Std. Error`[1]))) 
colnames(t.true.ar) = as.factor(length.list)
sapply(c(1:4), function(x) table(abs(t.true.ar[,x])>1.96))
d = t.true.ar %>% reshape2::melt()
p2 = ggplot(data = d, aes(x = value, col = variable))+
  theme_bw()+
  stat_ecdf()+
  scale_x_continuous(limits = c(0.0, 0.3), breaks = seq(0.0, 0.3,0.05))+
  ylab("CDF")+
  xlab("Std. Err")
ggsave(filename = paste0(path_results, "attribution0/Std_err_n_",true.model,".jpg"), plot = p2)
