...
for (l in c(30,60,90,120)) {
  ...
  for (i in 1:nb.sim) {
    ...
    sigma[i,1] = sd(a)/sqrt(2-2*alpha)
    sigma[i,2] <- mad(a)/sqrt(2-2*alpha)
    sigma[i,3] <- robustbase::Sn(a)/sqrt(2-2*alpha)
    sigma[i,4] <- robustbase::Qn(a)/sqrt(2-2*alpha)
    sigma[i,5] <- robustbase::scaleTau2(a)/sqrt(2-2*alpha)
  }
  ...
  jpeg(paste0(path_results,"attribution/variances/sim1", "phi = ", alpha, ", n = ", n0, ", offset = ", off.set, ".jpeg"),
